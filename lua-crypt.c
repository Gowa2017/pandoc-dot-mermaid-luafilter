...
static uint32_t LHs[16] = {0x00000000, 0x00000001, 0x00000100, 0x00000101,
...
static void padding_mode_table(lua_State *L) {
...
static void padding_add_iso7816_4(uint8_t buf[8], int offset) {
    buf[offset] = 0x80;
    memset(buf + offset + 1, 0, 7 - offset);
}

static int padding_remove_iso7816_4(const uint8_t *last) {
    int padding = 1;
    int i;
    for (i = 0; i < 8; i++, last--) {
        if (*last == 0) {
...
static void padding_add_pkcs7(uint8_t buf[8], int offset) {
    uint8_t x = 8 - offset;
    memset(buf + offset, x, 8 - offset);
}

static int padding_remove_pkcs7(const uint8_t *last) {
    int padding = *last;
    int i;
    for (i = 1; i < padding; i++) {
        --last;
...
static inline void check_padding_mode(lua_State *L, int mode) {
    if (mode < 0 || mode >= PADDING_MODE_COUNT)
        luaL_error(L, "Invalid padding mode %d", mode);
}

static void add_padding(
    lua_State *L, uint8_t buf[8], const uint8_t *src, int offset, int mode) {
...
static int remove_padding(lua_State *L, const uint8_t *last, int mode) {
    check_padding_mode(L, mode);
...
static int ldesencode(lua_State *L) {
    ...
    uint8_t       *buffer = tmp;
    if (chunksz > SMALL_CHUNK) { buffer = lua_newuserdatauv(L, chunksz, 0); }
    int i;
...
static int ldesdecode(lua_State *L) {
    ...
    uint8_t  tmp[SMALL_CHUNK];
    uint8_t *buffer = tmp;
    if (textsz > SMALL_CHUNK) { buffer = lua_newuserdatauv(L, textsz, 0); }
    for (i = 0; i < textsz; i += 8) { des_crypt(SK, text + i, buffer + i); }
...
static int ltohex(lua_State *L) {
    ...
    char          *buffer = tmp;
    if (sz > SMALL_CHUNK / 2) { buffer = lua_newuserdatauv(L, sz * 2, 0); }
    int i;
    for (i = 0; i < sz; i++) {
...
static int lfromhex(lua_State *L) {
    ...
    if (sz & 1) { return luaL_error(L, "Invalid hex text size %d", (int)sz); }
    char  tmp[SMALL_CHUNK];
    char *buffer = tmp;
    if (sz > SMALL_CHUNK * 2) { buffer = lua_newuserdatauv(L, sz / 2, 0); }
    int i;
...
static void read64(lua_State *L, uint32_t xx[2], uint32_t yy[2]) {
    ...
    if (sz != 8) { luaL_error(L, "Invalid uint64 x"); }
    const uint8_t *y = (const uint8_t *)luaL_checklstring(L, 2, &sz);
    if (sz != 8) { luaL_error(L, "Invalid uint64 y"); }
...
static int lhmac_hash(lua_State *L) {
    ...
    size_t         sz = 0;
    const uint8_t *x  = (const uint8_t *)luaL_checklstring(L, 1, &sz);
    if (sz != 8) { luaL_error(L, "Invalid uint64 key"); }
...
static int ldhexchange(lua_State *L) {
    size_t         sz = 0;
    const uint8_t *x  = (const uint8_t *)luaL_checklstring(L, 1, &sz);
    if (sz != 8) { luaL_error(L, "Invalid dh uint64 key"); }
    uint32_t xx[2];
    xx[0] = x[0] | x[1] << 8 | x[2] << 16 | x[3] << 24;
    xx[1] = x[4] | x[5] << 8 | x[6] << 16 | x[7] << 24;
...
static int lb64decode(lua_State *L) {
    ...
    for (i = 0; i < sz;) {
        ...
        int c[4];
        for (j = 0; j < 4;) {
            if (i >= sz) { return luaL_error(L, "Invalid base64 text"); }
            c[j] = b64index(text[i]);
            if (c[j] == -1) {
                ++i;
                continue;
        ...
        case 2:
            ...
            ++output;
            break;
        default: return luaL_error(L, "Invalid base64 text");
        }
    }
...
static int lxor_str(lua_State *L) {
    ...
    if (len2 == 0) { return luaL_error(L, "Can't xor empty string"); }
    luaL_Buffer b;
    char       *buffer = luaL_buffinitsize(L, &b, len1);
    int         i;
    for (i = 0; i < len1; i++) { buffer[i] = s1[i] ^ s2[i % len2]; }
    luaL_addsize(&b, len1);
    luaL_pushresult(&b);
    return 1;
}
...
LUAMOD_API int luaopen_crypt(lua_State *L) { return luaopen_skynet_crypt(L); }
